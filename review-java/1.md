...
<!-- GFM-TOC -->
<!-- GFM-TOC -->
*[1 Runtime类的使用](## 1.1 Runtime类的使用)
    *[ 1.1 Runtime类的使用](## 1.2 内存管理)
*[2 System类的使用](# 2 System类的使用)
    *[2.1 利用currentTimeMillis()记录程序执行的时间](## 2.1 利用currentTimeMillis()记录程序执行的时间)
<!-- GFM-TOC -->
...
## Date类和日期有关的类，系统中存在着两个同名的Date类，其中一个包括在java.util包中，另外一个在java.sql包中；这里介绍的是前者，作用就是获取当前时间
，另外也有设置时间以及一些其他的辅助功能。
